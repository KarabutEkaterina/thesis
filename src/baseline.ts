...
export async function baselineApy() : Promise<ValidatorData[]> {
    const wsProvider = new WsProvider('wss://rpc.polkadot.io');
    const api = await ApiPromise.create({provider: wsProvider});
    ...
    return  sortByYearlyReward(data, totalStake, balanceTotalIssuance.toBn())
    // let best = sorted.slice(0, 16)

    //console.log(best)
}

export async function bestBaselineApy(): Promise<ValidatorData[]>{
    var baselineAPY: ValidatorData[] = await baselineApy();
    return baselineAPY.slice(0, 16)
}